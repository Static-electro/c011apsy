...
namespace c011apsy
{
	...
	*/
	...
	bool Wave<T>::collapse( bool oneStep, Callback c )
	{
		...
		size_t uncertaintyCurrent = m_field.size();
		size_t id0 = getCollapsePoint( uncertaintyCurrent );

		while ( uncertaintyCurrent > m_field.size() )
		{
			collapseStep( id0, c );
			id0 = getCollapsePoint( uncertaintyCurrent );

			if ( oneStep )
			{
				return uncertaintyCurrent == m_field.size();
			}
		}
	...
	void Wave<T>::initRandom()
	{
		...
		m_mt = std::make_unique<std::mt19937>( m_seed.rndSeed );
	}
